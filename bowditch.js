...
function calculateMisclosureAndAccuracy() {
    ...
    // Generate results table after displaying misclosure and linear accuracy
    generateResultsTable();
}
...
function generateResultsTable() {
    ...
    window.lines.forEach((line, index) => {
        ...
        row.innerHTML = `
            ...
            <td>${line.latitude.toFixed(4)}</td>
            <td>${line.departure.toFixed(4)}</td>
            <td>${line.correctedLatitude ? line.correctedLatitude.toFixed(4) : 'N/A'}</td>
            <td>${line.correctedDeparture ? line.correctedDeparture.toFixed(4) : 'N/A'}</td>
        `;
    ...
    const popup = window.open('', 'Results', 'width=900,height=600');
    popup.document.open();
    popup.document.write(`
        ...
        <head>
            ...
            <style>
                ...
                .print-button { display: block; margin: 20px auto; padding: 10px 20px; font-size: 16px; background-color: #4CAF50; color: white; border: none; border-radius: 5px; cursor: pointer; }
